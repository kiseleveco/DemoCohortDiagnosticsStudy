...
runCohortMethod <- function(connectionDetails,
  ...
  write.csv(results, file.path(outputFolder, "analysisSummary.csv"), row.names = FALSE)
...
createTcos <- function(outputFolder) {
  ...
  createTco <- function(i) {
    ...
    outcomeOfInterest <- CohortMethod::createOutcome(outcomeId = outcomeIds,
                                     outcomeOfInterest = TRUE)
    negativeControlIds <- allControls$outcomeId[allControls$targetId == targetId & allControls$comparatorId == comparatorId]
    ...
    print(c(targetId, comparatorId, outcomeIds, excludeConceptIds, includeConceptIds))
