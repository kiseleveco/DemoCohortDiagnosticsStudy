...
exportAnalyses <- function(outputFolder, exportFolder) {
  ...
  tempFileName <- tempfile(fileext = "")

  cmAnalysisListFile <- system.file("settings",
                                    "cmAnalysisList.json",
                                    package = "DemoCohortDiagnosticsStudy")
  cmAnalysisList <- CohortMethod::loadCmAnalysisList(cmAnalysisListFile)
  cmAnalysisToRow <- function(cmAnalysis) {
    ParallelLogger::saveSettingsToJson(cmAnalysis$analysisId, tempFileName)
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "DemoCohortDiagnosticsStudy")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("Cohorts.csv", package = "DemoCohortDiagnosticsStudy")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(exposureId, ".json"), package = "DemoCohortDiagnosticsStudy")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  createOutcomeRow <- function(outcomeId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(outcomeId, ".json"), package = "DemoCohortDiagnosticsStudy")
    definition <- readChar(cohortFileName, file.info(cohortFileName)$size)
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "DemoCohortDiagnosticsStudy")
...
exportMetadata <- function(outputFolder,
  ...
  database <- tibble(databaseId = databaseId,
                     ...
                     studyPackageVersion = utils::packageVersion("DemoCohortDiagnosticsStudy"),
                     isMetaAnalysis = 0)
...
calibrate <- function(subset, allControls) {
  ...
  existing <- intersect(expected, colnames(subset))
  print(existing)
  ParallelLogger::logTrace(existing)

  subset <- subset[, existing, drop = FALSE]
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "DemoCohortDiagnosticsStudy")
